...
# Data 생성 class 정의
class DataGenerator:
    __train_dir__   = './mouse/train'
    __val_dir__     = './mouse/val'
...
class Model:
    ...
    __model_dir__   = './mouse/model'
    __test_dir__    = './mouse/test'
...
if __name__ == "__main__" :
    ...
    resnet_without_augment.make_pretrained_model()
    # fine tuning을 위한 모델 구성
    resnet_without_augment.make_finetuned_model()

    # 학습 : 모델명이랑 hdf5 파일명이랑 동일하게 해주세요.
    resnet_without_augment_history = resnet_without_augment.training('resnet_without_augment.hdf5',
                                                                    train_generator_without_augment,
                                                                    val_generator_without_augment)

    # 학습결과 시각화
    resnet_without_augment.plot(resnet_without_augment_history)

    # 기능1 구현(이미지 분류)
    resnet_without_augment.classify_img()
